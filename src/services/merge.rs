...
use crate::models::{branch::Branch, repo::Repository, Accessible};
...
pub trait MergeService {
  ...
  fn get_merge_base(commit1: &Commit,
                    commit2: &Commit) -> anyhow::Result<Commit>;
}
...
impl MergeService for Repository {
  ...
  fn merge(&self, theirs_branch: &Branch) -> anyhow::Result<()> {
    ...
    let ours_branch = repo.head().load_branch()?.unwrap();
    ...
    let base_tree = repo.wrap(Object::accessor(&base_commit.tree)).load()?.unwrap().cast_tree();
    let ours_tree = repo.wrap(Object::accessor(&ours_commit.tree)).load()?.unwrap().cast_tree();
    let theirs_tree = repo.wrap(Object::accessor(&theirs_commit.tree)).load()?.unwrap().cast_tree();
    ...
    if !conflicts.is_empty() {
      for conflict in conflicts {
        if conflict.line_start == conflict.line_end {
          println!("Merge conflict in {}: {}", conflict.file, conflict.line_start);
        } else {
          println!(
            "Merge conflict in {}: [{} , {}]",
    ...
    // ✅ 无冲突，生成合并提交
    let merged_tree_obj = Object::Tree(merged_tree);
    merged_tree_obj.save()?;
    let tree_sha1 = merged_tree_obj.sha1().into();
  ...
  fn get_merge_base(commit1: &Commit, commit2: &Commit) -> anyhow::Result<Commit> {
    let repo = Repository::load()?;
    ...
    while let Some((source, sha)) = queue.pop_front() {
      ...
      *state |= source;
      ...
      if *state == 3 {
        // 从两个方向都访问到了，找到 LCA
        let obj = repo.wrap(Object::accessor(&sha)).load()?.unwrap();
      ...
      // 向上继续遍历父节点
      let Object::Commit(commit) = repo.wrap(Object::accessor(&sha)).load()?.unwrap() else {
        anyhow::bail!("Object {} is not a commit", sha);
      };
